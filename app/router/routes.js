...
function verifyToken(token_string, callback) {
    ...
    if (token_string === undefined || token_string.length == 0) {
        callback(true, {
            ...
            error: 'token_string is undefined'
        })
    } else {
...
function loginUser(req, res) {
    ...
    if (!(typeof email === 'string' && typeof password === 'string')) {
        res.json({
            success: false,
            message: "Please check if you filled out everything"
        })
    } else {
        if (validator.isAlphanumeric(password) &&
            ...
            Auth.findOne({
                ...
                if (err || authdata === null) {
                    ...
                    res.json({
                        success: false,
                        message: "This email is not registered with Snippet"
                    })
                } else {
                    if (authdata.fb_authed) {
                        ...
                        res.json({
                            success: false,
                            message: "This email is in use through Facebook"
                        })
...
function registerUser(req, res) {
    ...
    if (!(typeof username === 'string' && typeof email === 'string' && typeof password === 'string' && typeof passwordCheck === 'string')) {
        ...
        res.json({
            success: false,
            message: "Password must only include alphanumeric characters"
        })
        ...
        res.json({
            success: false,
            message: "Password must include 8 to 64 characters"
        })
        ...
        newUser.save(function (err) {
            if (err) {
                ...
                res.json({
                    success: false,
                    message: "This email is already registered with Snippet"
                })
            } else {
                // new user, add auth token and pass to index
                var newAuth = new Auth({
                ...
                if (req.body.username.includes("@")) {
                    ...
                    User.find({
                        ...
                        if (err) {
                            ...
                            var result = users.filter(function (current) {
                                return userdata.friends.filter(function (current_b) {
                                    return current_b == current.email
                                }).length == 0

                            });

                            res.json(result.filter(function (u) {
